...
export default class extends Component {
  ...
  render() {
    ...
    const onFinishButtonLabel =
      finishButtonLabel ||
      (!processWithCloudService && !processWithFilerobot ? t['toolbar.download'] : t['toolbar.save'])
    const applyAndSave = () => {
      apply(handleSave)
    }
      ...
      <HeaderWrapper>
        <HeaderTop>
          ...
          <ButtonsWrapper>
            ...
            {showSaveAs && <SaveActions t={t} handleSaveAs={applySaveAsHandle} />}
            <CancelBtn
              hide={!activeTab}
            ...
            <div className="my-btn-container">
              ...
              <Button
                disabled={selectedShape.index === undefined}
                primary={textTheme === 0}
                onClick={() => this.updateProperty('textTheme', 0)}
              >
                黑底白字
              </Button>
              <Button
