...
export default class CustomizedCanvas extends Component {
  ...
  addImage = ({
    ...
    if (img) {
      const addIt = () => {
        ...
        if (others.key && this.replaceShapeIfExisted(others.key, allArgs, otherStates)) {
          return
        }
  ...
  render() {
    ...
    /** 希望的交互： 可添加区域块，可拖动，可放大缩小。
     * 1. 只留4个边界点
     * 2. 添加的文字框宽度要大小适中
     * 3. 对4个边界点添加拖动操作
     * 4. 交互改进，当前需要点击一下选中后才能拖动。
     * 5. 【优先】下次可做，黑色背景的上下边距基于当前文字的坐标来处理，
     *
     * 要修改的相关文件：
     * /ToolBar/Text.js
     * /CustomizeCavas.js
