...
class Select extends Component {
  ...
  getValue = (value) => {
    ...
    label = processValue
      ...
      : item && (item[labelProp] && renderLabel ? renderLabel(item[labelProp]) : item[labelProp])
  ...
  filterList = (list) => {
    ...
    if (exclude && exclude.length) return list.filter((item) => !exclude.includes(item[valueProp]))
  ...
  render() {
    ...
    const {
      ...
      size,
      small = false,
      notRelativePosition = false,
    } = this.props
    const getLabel = (item) =>
      item[labelProp] && renderLabel ? renderLabel(item[labelProp]) : item[labelProp]
    ...
    return (
      ...
      >
        ...
        >
          {this.getValue(value)} {labelDescription ? `(${labelDescription})` : ''}
        </SelectedItem>
        ...
        >
          {this.filterList(list).map((item) => (
            <SelectDropdownItem
              ...
              key={item[valueProp]}
              onClick={(event) => {
                this.onItemClick(event, item[valueProp])
              }}
            >
...
const SelectDropdown = styled.ul`
  ...
  box-shadow: inset 0 1px 1px rgba(0, 0, 0, 0.5), 0 1px 0 rgba(82, 104, 109, 0.6);
