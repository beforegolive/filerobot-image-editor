...
export default class extends Component {
  ...
  loadImage = () => {
    ...
    if (watermark && watermark.url) {
      ...
      logoImage.src =
        watermark.url + (watermark.url.indexOf('?') > -1 ? '&' : '?') + new Date().getTime()
    }
    ...
    if (!src.startsWith('data:image/') && !src.startsWith('blob:')) {
      ...
      img.src =
        img.src + (img.src.indexOf('?') > -1 ? '&version=' : '?version=') + new Date().getTime()
    }
    ...
    img.onload = () => {
      ...
      const propsOnApply = {
        ...
        logoImage,
        imageName:
          imageName.indexOf('?') > -1 ? imageName.slice(0, imageName.indexOf('?')) : imageName,
      }
  ...
  determineImageType = () => {
    ...
    xhr.onload = ({ target }) => {
      ...
      this.setState({
        imageMime: imageType(new Uint8Array(target.response))?.mime || 'image/svg+xml',
      })
    }
  ...
  onPreResize = (value) => {
    ...
    switch (value) {
      ...
      case 'resize':
        ...
        this.setState(
          { preCanvasDimensions: canvasDimensions, isPreResize: true, activeBody: 'preview' },
          () => {
            this.setState({ activeTab })
          }
        )
