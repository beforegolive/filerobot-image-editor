...
class ImageEditorWrapper extends Component {
  ...
  constructor({ show = false, src = '', config = {} }) {
    ...
    config.language =
      config.translations[config.language] || translations[config.language] ? config.language : 'en'
  ...
  close = (closingStatus = ON_CLOSE_STATUSES.CLOSE_BTN_CLICKED) => {
    ...
    const status =
      typeof closingStatus === 'object' ? ON_CLOSE_STATUSES.CLOSE_BTN_CLICKED : closingStatus
